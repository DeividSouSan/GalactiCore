...
beforeAll(async () => {
    ...
    await database.createQueryBuilder()
        ...
        .values([
            {
                name: "Sistema Solar",
                description: "Sistema no qual planetas orbitam uma estrela média."
            }
        ]).execute();
...
test('GET_planets_should_return_200', async () => {
    ...
    expect(responseBody.data.planets.length).toEqual(responseBody.data.count_planets);
...
test('POST_invalid_body_should_return_400', async () => {
    ...
    expect(responseBody).toMatchObject({
        status: 'error',
        message: "Corpo da requisição inválido. O planeta deve conter somente 'name', 'weather', 'terrain', 'population' e 'stellarSystemId'.",
        data: {}
    });
...
test('PUT_invalid_body_should_return_400', async () => {
    ...
    expect(responseBody).toMatchObject({
        status: 'error',
        message: "Corpo da requisição inválido. planet deve conter somente 'name', 'weather', 'terrain', 'population' e 'stellarSystemId'.",
        data: {}
    });

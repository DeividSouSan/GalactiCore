...
export class CreateTables1738878624522 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`planet\` (\`id\` int NOT NULL AUTO_INCREMENT, \`name\` varchar(255) NOT NULL, \`weather\` varchar(255) NOT NULL, \`terrain\` varchar(255) NOT NULL, \`population\` int NOT NULL, \`stellarSystemId\` int NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`stellar_system\` (\`id\` int NOT NULL AUTO_INCREMENT, \`name\` varchar(255) NOT NULL, \`description\` varchar(255) NOT NULL, UNIQUE INDEX \`IDX_e86c3c2ccdd88b9998bbacb859\` (\`name\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`spaceship\` (\`id\` int NOT NULL AUTO_INCREMENT, \`model\` varchar(255) NOT NULL, \`manufacturer\` varchar(255) NOT NULL, \`capacity\` int NOT NULL, UNIQUE INDEX \`IDX_eb821088410ce02088b7409b39\` (\`model\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`DROP INDEX \`IDX_eb821088410ce02088b7409b39\` ON \`spaceship\``);
        await queryRunner.query(`DROP TABLE \`spaceship\``);
        await queryRunner.query(`DROP TABLE \`character\``);
        await queryRunner.query(`DROP INDEX \`IDX_e86c3c2ccdd88b9998bbacb859\` ON \`stellar_system\``);
